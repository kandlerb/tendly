...
| `(landlord)/properties/add.tsx` | Add property form (address, nickname, mortgage, unit count, rent); error handling |
...
| `properties.ts` | `usePropertiesStore` — properties[], fetchProperties, addProperty, addUnit, error |
...
| File | Exports |
|------|---------|
| `alert.ts` | `showAlert(title, message?)` — uses window.alert on web, Alert.alert on native |

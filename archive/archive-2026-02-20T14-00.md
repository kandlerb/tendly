...
| `(auth)/login.tsx` | Email/password login; brand-color header on narrow, wordmark above card on wide; forgot-password flow |
| `(auth)/signup.tsx` | Landlord signup; brand-color header on narrow, wordmark + benefit list above form on wide |
| `(landlord)/_layout.tsx` | 5-tab bar on mobile (Help tab hidden via href:null), sidebar at ≥breakpoints.md; uses color tokens (no inline hex) |
...
| `(landlord)/tend.tsx` | Help — local 3-stage decision tree (home → topic list → answer); 5 topics, no AI calls |
...
| `claude.ts` | `inviteTenant`, `completeInvitation`, `getSignedUrl`, `acknowledgeDocument` (callTriage + callDraftMessage removed) |
...
| 2026-02-21T00:00 | (none) | CI fully green (quality 40s + deploy 1m23s). App live at https://kandlerb.github.io/tendly/ via peaceiris/actions-gh-pages on gh-pages branch. |
| 2026-02-21T17:00 | app.json | Fixed GitHub Pages blank page: expo export defaults asset paths to / but Pages serves from /tendly/. Added experiments.baseUrl: "/tendly" (not web.baseUrl — wrong key). JS bundle now resolves to /tendly/_expo/... and returns 200. |
| 2026-02-21T18:00 | app/(auth)/login.tsx, app/(auth)/signup.tsx, app/(landlord)/tend.tsx, app/(landlord)/_layout.tsx, app/(tenant)/maintenance.tsx, lib/claude.ts, supabase/functions/invite-tenant/index.ts | Professional polish & AI removal: (1) Auth screens — added full-width brand-color header block (wordmark + tagline) on narrow, wordmark above card on wide, benefit list on signup wide, forgot-password TouchableOpacity on login that calls resetPasswordForEmail; (2) tend.tsx fully rewritten as local 3-stage decision tree (home → topic → question → answer) with 5 topics and 4 questions each, no network calls, ChevronRight navigation, external link support for legal resources; (3) _layout.tsx — "Tend AI" tab label → "Help"; (4) maintenance.tsx — removed callTriage import, added synchronous triageLocally() with keyword matching for urgency (emergency/urgent/routine) and trade (7 categories); (5) lib/claude.ts — removed callTriage and callDraftMessage exports; (6) invite-tenant — redirectTo changed from native deep link to web URL https://kandlerb.github.io/tendly/onboarding?token=... |
